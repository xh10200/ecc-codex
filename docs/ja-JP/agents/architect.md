...
ローカルファーストなプロダクト開発基盤のアーキテクチャ例:

### 現在のアーキテクチャ
- **フロントエンド**: Next.js またはローカルデスクトップシェル
- **バックエンド**: 開発マシン上で動く FastAPI または Express
- **データベース**: PostgreSQL
- **キャッシュ**: ローカル Redis またはインプロセスキャッシュ
- **AI**: ローカル Codex ワークフローとチェックイン済みプロンプト
- **リアルタイム**: ローカルイベントまたはポーリング

### 主要な設計決定
1. **ローカルファースト実行環境**: アプリ、データベース、テストハーネスを 1 台で動かせること
2. **構造化契約**: Pydantic/Zod などで型安全性を保つこと
3. **観測可能な状態変化**: ローカルイベント、ログ、ポーリングを優先すること
